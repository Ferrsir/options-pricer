...
* **Two volatilities, two names.** The **model σ** is the number in the σ box. When you load a ticker the script fills it with the market's 30-day implied vol (its suggestion); you can overwrite it with your own view. It sets the *model fair value*. **My premium's σ** is the volatility implied by the premium *you typed*. "My" belongs to the premium because that is the number you supply.
* **Implied vol:** the volatility that would make the formula produce *exactly your premium* (my premium's σ). Compare it to the model σ: if implied vol is higher, your premium is pricing in **more** movement than the model assumes.
* **Recommended premium:** when you load a stock, the premium box is filled in for you with what the model says the option *should* cost at the model σ (an American option, priced with the binomial tree). It follows the recommendation while you change the strike, days or σ. As soon as you type your own number (or drag the slider, use the arrow keys or the % chips) it stops following, and everything updates around *your* number. "Use recommended" puts the model's price back.
* **Expiry and contract lists (under "Advanced"):** completely optional. They list the real options traded on that stock, so you can copy a real strike, days left and market price instead of typing them. You never need them.
* **Greeks, surfaces and scenarios use: My premium's σ / Model σ.** A premium different from the model price implies a different volatility. This switch chooses whether the Greeks tab, the 3-D Greek surface, the payoff "today" curve and the What-if tab are computed at that implied volatility (the default once you enter a premium) or at the model σ. The fair value always uses the model σ. The σ box is never overwritten. This is how desks work: **they quote volatility, not dollars.** When a desk says "bid at 12.3", that is a volatility. On the reference option 12.3% vol is $7.61.
...
"Edge" is not a guarantee: it says the *model, at the model σ*, thinks the price is off. If the model σ is wrong, so is the edge. This is why the What-if tab and the implied vol matter.
...
| | Model σ = 20% | My premium (σ implied by 150) | Change |
|---|---|---|---|
...
* **Edge:** at the model σ you'd overpay by 45. At my premium's σ the "edge" is zero *by construction*. The decision is whether my premium's σ is reasonable.

**The six curves** below the table show the same idea continuously: the horizontal axis is the premium, and every point re-solves the implied vol and recomputes the Greeks at it. The white dashed line is the model price at the model σ; the orange line is your premium. You can watch delta, gamma, vega, theta and P(profit) slide as the premium moves from cheap to expensive. (For extremely high premiums the implied vol explodes because the premium approaches its upper bound, the stock price itself; the charts stop before that.)
...
**What changes when you edit the premium.** With the switch on *My premium's σ* (the default), the **Greeks** tab overlays two sets of lines: dashed = the model σ, solid = the σ your premium implies, so the gap is exactly what the premium changes. The **3-D Greek surface** has a *View* menu: at my premium's σ, at the model σ, or their **difference** (red = the premium lowers that Greek, green = raises it). The **payoff** adds a dashed "today at model σ" curve, **What-if** values the scenarios at my premium's σ, and the **Vol surface** tab drops a marker where your premium's implied vol sits on the market surface (compare only if your inputs describe the same stock or index). Switch to *Model σ* to put all of that back on the model σ.
...
7. **Type a premium, then open the Greeks tab**: dashed lines = the model σ, solid = my premium's σ. Open the 3-D Greek surface and change *View* to **Difference** to see where in (spot, time) the premium changes each Greek most.
...
| 16 · Takeaways | Quote in vol; convexity is the product | My premium's σ switch, What-if |
...
* **"The banner says my premium is 'rich' (or 'cheap'), but it is just the normal market price. Why?"** The model prices with **one flat σ**. Real markets charge a different vol for every strike and every expiry. Two effects cause most of the gap. **Skew:** out-of-the-money puts trade at a higher vol than at-the-money options, because people pay extra for crash protection. On 2026-09-18 an SPY put 5% below the price, 42 days out, had an implied vol of 17.1% while the at-the-money options at that expiry were about 13.3%; against a flat 11.7% the put looked "264% rich", but it was simply the going price. **Term structure and events:** on the same day PLTR's at-the-money vol was about 46% for 20 to 30 days out and about 57% for 63 days out, most likely because an earnings report falls in between. To judge whether a premium is out of line, compare it with the market's vol *at that strike and expiry*: the dot on the **Vol surface** tab is exactly that comparison.
