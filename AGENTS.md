...
2. **Bump the cache-bust version** (`?v=N`) in `docs/index.html`, `docs/js/app.js` (imports and the `guide.html` fetch) and `docs/js/data.js` whenever you change anything under `docs/`. GitHub Pages caches for 10 minutes; without the bump visitors get a half-stale site. Current value: `v=13`.
...
10. **Two volatilities, never merged, and named for who supplies them.** In the UI the σ box is the **model σ** (the script pre-fills it with the 30-day implied vol; the user may overwrite it) and the σ implied by the typed premium is **my premium's σ** ("my" = the number the user typed). Never call the σ box "my σ" again (the owner objected on 2026-09-18). In code: `readRaw()` = the inputs as typed; `readInputs()` adds `p.sigmaModel` (the σ box) and sets `p.sigma` to the σ implied by the premium when `state.basis === 'market'` (the default once a premium is valid). `cache.mod` holds prices at your σ (hero, Overview prices, banner edge, LSMC); `cache.*` (g, ag, bsm, eu, am) is the analysis basis used by Greeks, payoff, What-if, Tree and both surfaces. The implied vol is **never written back into the σ input** (the old lock checkbox did, which destroyed the comparison).
...
### 2026-09-18 · Claude → Codex (American Options Pricer: simpler main flow)
Done: The owner said the site was overcomplicated (they did not know what the Expiry/Contract lists were) and that what they need is the "American Options Pricer": load a stock, get a recommended premium, change it, watch options, Greeks and surfaces react. So: (1) branding and page title are now "American Options Pricer" and **American is the default exercise style**; (2) loading a ticker pre-fills the premium box with the model's price (`state.premAuto`, `syncAutoPremium()`); it keeps following strike/days/σ changes until the user types, drags the slider, uses a % chip or picks a real contract, and "Use recommended" (or the "= model" chip) turns following back on; presets and share links set an explicit premium so they never auto-follow; (3) ticker load also sets Days to the first listed expiry at least 25 days out (K stays at the rounded spot); (4) the real Expiry/Contract lists moved into a closed "Advanced: copy a real listed option" section; picking an expiry now also recomputes (it used to leave results stale). Guide and README updated. Cache-bust `v=13`. NOT removed: the other tabs and the skew note; ask the owner if they want the site cut down further.
Verified (local dev server, live Cboe data): AAPL load gives K=335, 27.7 days, model σ 22.45%, recommended premium 8.6997 (American tree) with implied vol equal to the model σ; strike 350 makes the premium follow (3.198); typing 4.80 stops the following (banner says "Your premium", implied vol 27.56%); changing the strike afterwards leaves 4.80 alone; "Use recommended" restores following; all eight tabs render with a typed premium under American, no JS errors, no NaN text. 31 tests pass. A syntax slip in my first patch (unclosed brace in the chip handler) was caught by the browser console before commit; there is still no automated JS parse check (idea: a headless-Edge smoke test in `scripts/`). NOT verified: Safari/Firefox, phones, the American Greeks charts (still Black-Scholes, labelled).
Needs from you: nothing.
Open questions: none.

### 2026-09-18 · Claude → Codex (σ naming + surface covers your contract)
Done: (1) The owner pointed out the σ box was called "my σ" although the script fills it (30-day implied vol) while the thing they type is the premium. Renamed everywhere in the UI, Guide and rule 10: **Model σ** = the σ box ("Model volatility σ", editable), **My premium's σ** = the vol implied by the typed premium. Switch buttons, Greek-surface View menu, chart legends, notes, tooltips and the Impact table headers all follow. (2) The owner's screenshot showed the "your premium" dot floating away from the vol surface. Cause: the surface was built from chains 14 to 160 days out and -0.15..+0.10 log-moneyness, so a long-dated (e.g. 272-day) or far-strike contract fell off its edge and the axis stretched to reach it. Now `getSurface(ticker, {maxDays, kRange})` asks `/api/chains` for expiries out to the contract's expiry (+30 days, API caps 800 days / 20 expiries) and a wider strike range, and `maybeWidenSurface()` rebuilds it once when you pick a contract the surface does not cover. `surfaceCovers()` has a 0.005 tolerance so a contract at the edge is not flagged. Only the serverless-chains path honours this; the local `python -m pricer serve` surface (Python) is unchanged. Cache-bust `v=11`.
Verified (local dev server, live Cboe data): PLTR 272-day call: surface T range 0.057 to 0.744, marker 58.2% vs surface 57.3%, no "outside" note; PLTR 63-day put at K/S=0.79: surface k range -0.25..0.10, marker 60.9% vs 60.3%; near-term call unchanged (0.057 to 0.421). New labels checked in the DOM. 31 tests pass. NOT verified: a picture of the fixed surface (the in-app pane's screenshots do not track scrolling), the 3-D text label clipping seen in the owner's screenshot ("your pre..."; likely fixed only because the dot now sits inside the axes), the local Yahoo-based surface path, Safari/Firefox, phones.
Needs from you: nothing.
Open questions: none.
