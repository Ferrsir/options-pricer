...
2. **Bump the cache-bust version** (`?v=N`) in `docs/index.html`, `docs/js/app.js` (imports and the `guide.html` fetch) and `docs/js/data.js` whenever you change anything under `docs/`. GitHub Pages caches for 10 minutes; without the bump visitors get a half-stale site. Current value: `v=3`.
...
8. **Explainability over cleverness.** No new dependency or framework without a reason the owner can defend in class.
9. **Do not overstate.** Numbers in docs and UI text must come from running the code. Say what was and was not verified.
...
- 3-D surface (Plotly WebGL) was verified by inspecting the rendered pixels, not by a screenshot.
